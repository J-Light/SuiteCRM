...
function insertLineItems(product,group){
	...
	for(var p in product){
		if(document.getElementById(type + p + ln) !== null){
			if(product[p] !== '' && isNumeric(product[p]) && p != 'vat'  && p != 'product_id' && p != 'name' && p != "part_number" && p != "cost_currency_c" && p != "cost_2_c" && p != "account_number_sales_c" && p != "account_number_purchase_c" && p!="job_number_text_c" && p!="tax_c" && p!='license_type_c' && p!='status_c' && p != 'is_ansys'){
				...
				document.getElementById(type + p + ln).value = format_money(removeCurrency(product[p]));
	...
	calculateLine(ln,type, true, true);
...
function adjustStartEndDate(lineno, months) {
	/*var _startDate = $('#product_start_date_c'+lineno).val();
	 ...
	 _startDate = parseDate(_startDate);;
	 }*/
 ...
 */
function computeLeaseAnsys(annual_price, days) {
	annual_price = isNaN(annual_price) ? 0 : annual_price;

	var new_price = 0;
	var daily_rate = annual_price / 365;
 ...
 */
function computeLeaseNotAnsys(annual_price, days) {
	annual_price = isNaN(annual_price) ? 0 : annual_price;
 ...
 * Compute Lease Line Items - NOT ANSYS
 */
function computeMaintenance(lineno, annual_price) {
	var _startdate = $('#product_start_date_c'+lineno).val();
	var _enddate = $('#product_end_date_c'+lineno).val();
...
function insertProductLine(tableid, groupid) {
	...
	b5.innerHTML = "<span class='dateTime'><input style='width:115px;' class='date_input' autocomplete='off' type='text' name='product_end_date_c[" + prodln + "]' id='product_end_date_c" + prodln + "' maxlength='50' value='' title='' value='' readonly='readonly' >" +
	...
	f.innerHTML += "<select tabindex='116' name='product_vat[" + prodln + "]' id='product_vat" + prodln + "' onchange='calculateLine(" + prodln + ",\"product_\", true, true);'>" + vat_hidden + "</select>";
	cellno_row1++;
	...
	k.innerHTML += "<input type='text' name='product_supplier_margin_c[" + prodln + "]' id='product_supplier_margin_c" + prodln + "' onchange='calculateLine(" + prodln + ",\"product_\");'></input>";
	cellno_row1++;
	...
	k.innerHTML += "<input type='text' name='product_cost_discount_c[" + prodln + "]' id='product_cost_discount_c" + prodln + "' onchange='calculateLine(" + prodln + ",\"product_\");' style='text-align:right;'></input>";
	cellno_row1++;
	...
	k.innerHTML += "<input style='text-align: right; width:115px;' type='text' name='product_supplier_amount_c[" + prodln + "]' id='product_supplier_amount_c" + prodln + "'>" +
		"</input>";
	cellno_row1++;
	...
	if(_license_type == 'Paid Up') {
		...
		$.ajax({
			...
			success: function(ret) {
				...
				var newlineno = insertProductLine("product_group0","0");
				...
				//$('#product_license_type_c'+newlineno).val(12);
...
function changeEndDate(ln, key){
	...
	console.log(days);
	calculateLine(ln, key, true, false)
}
...
function changeSalePrice(ln, key) {
	...
	calculateLine(ln, key, false, false)
}
...
function calculateLine(ln, key, boolsaleprice = true, boollistprice = true, boolenddate = true){
	var module = document.getElementById('_module').value;
	...
	if(document.getElementById(key + 'product_list_price' + ln) !== null && document.getElementById(key + 'product_discount' + ln) !== null && document.getElementById(key + 'discount' + ln) !== null){
		...
		if (license_type == 'Lease' || license_type == 'Maintenance') {
		...
		if(boollistprice) {
			...
			if(boolenddate) {
				if (license_type == 'Lease') {
					if (is_ansys == 1) {
						listPrice = computeLeaseAnsys(fix_new_price, days);
						cost_2_c = computeLeaseAnsys(cost_2_c, days);
					} else {
						listPrice = computeLeaseNotAnsys(fix_new_price, days);
						cost_2_c = computeLeaseNotAnsys(cost_2_c, days);
					}
				}
	...
	if(module == 'AOS_Quotes') {
		...
		var quoted_cost = ((unformat2Number(cost_2_c) * supplier_margin)) - (((unformat2Number(cost_2_c) * supplier_margin)) * cost_discount);
	...
	if(module == 'AOS_Quotes') {
		var margin = (productUnitPrice > 0 && isNumeric(productUnitPrice)) ? format2Number(((productUnitPrice - converted_cost) / converted_cost) * 100) : 0;
		margin = isNumeric(margin) ? margin : '0.00';
		margin += '%';
...
function calculateTotal(key)
{
	...
	for (i=0; i < length; i++) {
		...
		} else if (qty !== 0 && list !== null && deleted != 1) {
			//tot_amt += list * qty;
			sale_tot_amt_2 += list * qty;
		}
	...
	set_value(key+'total_amount',sale_tot_amt_2 + tax + shipping);
}
